"""
MODULE: Z.ai Adapter Client
PURPOSE: Interface for interacting with the Z.ai LLM API using OpenAI-compatible SDK.
INVARIANTS: Resilient to missing API keys at initialization; provides retry logic for rate limits.
HOW TO MODIFY: Update Chat parameters in the chat() method or adjust retry strategy.
"""
import os
import httpx
import logging
...
class ZaiClient:
    ...
    def _init_client(self):
        ...
        logger.info(f"ZaiClient initialized.")
    ...
    async def _ensure_client(self):
        ...
        if not self._client:
            ...
            if key and key != "EMPTY_KEY":
                ...
                return self._client
            raise ValueError("ZaiClient is not configured. Please set ZAI_API_KEY.")
        return self._client
