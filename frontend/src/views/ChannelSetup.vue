  ...
  <div class="min-h-[calc(100vh-64px)] w-full bg-white font-inter text-slate-700 pb-20 relative overflow-hidden flex flex-col">
    ...
    <div class="p-5 border-b border-slate-200 bg-white border border-slate-200 shadow-sm rounded-b-[2rem] sticky top-0 z-30 mb-4 relative">
       <div class="flex items-center gap-3 mb-2">
         ...
         <div>
           <h1 class="text-2xl font-bold text-slate-900 tracking-tight leading-none">Channel Setup</h1>
           <p class="text-[10px] text-blue-600 font-bold uppercase tracking-widest mt-1">Messaging Connectivity</p>
         </div>
       </div>
    </div>
    ...
    <main class="flex-grow px-4 pb-10 relative z-10 w-full max-w-2xl mx-auto space-y-6">
      ...
      <!-- Status Banner -->
      <div v-if="message" class="bg-white border border-slate-200 shadow-sm p-4 rounded-2xl border border-indigo-500/30 bg-indigo-500/10 flex items-center justify-center gap-2">
      ...
      <!-- Main Config Card -->
      <div class="bg-white border border-slate-200 shadow-sm rounded-3xl border border-slate-200 p-1 overflow-hidden relative">
         <div class="bg-slate-50 p-5 rounded-[1.35rem]">
            
            <div class="flex items-center justify-between mb-6">
               <h2 class="text-sm font-black uppercase tracking-widest text-slate-900">WhatsApp Config</h2>
            ...
            <div class="space-y-4 mb-6">
               <div class="space-y-1.5">
                  <label class="text-[10px] uppercase tracking-widest font-bold text-slate-600 pl-1">Session Key</label>
                  <input v-model="waSessionKey" type="text" placeholder="e.g. primary" class="w-full bg-white border border-slate-200 rounded-2xl px-4 py-3 text-sm text-slate-900 focus:outline-none focus:border-purple-500 transition-colors" />
               </div>
               <div class="space-y-1.5">
                  <label class="text-[10px] uppercase tracking-widest font-bold text-slate-600 pl-1">Display Name</label>
                  <input v-model="waDisplayName" type="text" placeholder="e.g. Primary WhatsApp" class="w-full bg-white border border-slate-200 rounded-2xl px-4 py-3 text-sm text-slate-900 focus:outline-none focus:border-purple-500 transition-colors" />
               </div>
            </div>

            <!-- Session Selector & Status Display -->
            <div class="bg-white rounded-2xl p-4 border border-slate-200 mb-6 relative overflow-hidden">
               ...
               <div class="flex items-center justify-between mb-3 pl-2">
                  <span class="text-xs font-bold text-slate-600">Current Session</span>
               ...
               <div class="relative pl-2">
                  <select
                     ...
                     class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-semibold text-slate-900 focus:outline-none focus:border-purple-500 transition-colors appearance-none"
                  >
            ...
            <!-- QR Code Display -->
            <div v-if="waQrImage" class="bg-white border border-slate-200 shadow-sm rounded-2xl border border-blue-500/30 p-5 flex flex-col items-center justify-center text-center space-y-4 mb-6 shadow-[0_0_30px_rgba(59,130,246,0.15)] relative overflow-hidden">
               <div class="absolute inset-0 bg-blue-500/5 pointer-events-none"></div>
               <div class="relative z-10 w-full max-w-[200px] aspect-square bg-white rounded-xl p-3 flex items-center justify-center border-4 border-slate-300">
                  <img :src="waQrImage" alt="WhatsApp QR" class="w-full h-full object-contain mix-blend-multiply" />
               </div>
               <div class="relative z-10">
                  <h3 class="text-sm font-bold text-slate-900 tracking-tight">Scan to Connect</h3>
                  <p class="text-xs text-slate-600 mt-1">Open WhatsApp on your phone and link device.</p>
               </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
               <button 
                  ...
                  class="col-span-2 py-3.5 bg-blue-600 text-white shadow-sm hover:bg-blue-700 rounded-xl font-bold text-sm shadow-lg shadow-purple-500/20 active:scale-[0.98] transition-transform disabled:opacity-50 flex items-center justify-center gap-2"
               >
               ...
               <button 
                  ...
                  class="py-3 bg-slate-100 border border-slate-200 text-slate-700 rounded-xl font-semibold text-xs active:scale-95 transition-transform disabled:opacity-50 flex items-center justify-center gap-1.5"
               >
